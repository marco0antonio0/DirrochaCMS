...
> **Status da documentação.** Esta documentação e o material em `paper/` e `docs/` são um
> rascunho em revisão ativa, não uma versão final. Redação, estrutura e algumas afirmações ainda
> estão sendo revisadas. Correções são bem-vindas por
...
- [Uso Academico e Submissao ao JOSS](#uso-academico-e-submissao-ao-joss)
- [Changelog](#changelog)
...
- Autenticação com JWT e sessão persistida e revogável.
- Proteção anti-bot self-hosted com ALTCHA no login e na criação da primeira conta.
- Gestão de contas do painel por papéis (`admin` / `editor` / `viewer`), com criação, alteração de senha, desativação e exclusão.
...
O DirrochaCMS usa uma arquitetura monolítica full-stack: frontend
administrativo, rotas HTTP de API e módulos internos de backend ficam na mesma
aplicação Next.js. O Firestore permanece externo e é acessado apenas pelo
servidor via Firebase Admin SDK.

![Arquitetura monolítica do DirrochaCMS](./images/architecture-monolith.svg)
...
├── docs/                    # Guia de verificação e material do JOSS
├── examples/                # Casos de uso documentados
├── paper/                   # Artigo para o JOSS
...
Depois apague o JSON baixado, já que a chave está no `.env`:
...
5. Ao escolher `Privado`, informe uma senha manualmente ou use `Randomizar` para gerar uma senha
   segura. O valor é exibido uma única vez e guardado como HMAC, então copie antes de fechar.
...
1. Acesse `/configuration`.
2. Na seção `Contas do painel`, preencha nome, e-mail, senha e papel para criar uma conta.
3. Use `Editar` para alterar nome, e-mail, papel ou definir uma nova senha.
4. Use `Desativar` para bloquear o acesso sem excluir a conta.
5. Use `Excluir` para remover definitivamente uma conta.

Papéis e permissões:

| Papel | Ler | Escrever | Excluir | Gerenciar contas |
| --- | :---: | :---: | :---: | :---: |
...
- Requisições que alteram estado validam `Sec-Fetch-Site`, com fallback de `Origin` contra
  `Host` (proteção CSRF).
- As tentativas de login têm limite de taxa por conta e por endereço IP.
...
- Senhas precisam ter no mínimo 10 caracteres e são recusadas se constarem numa lista de
  senhas comuns.
...
4. Teste manualmente os fluxos alterados — veja
   [`docs/manual-verification.md`](./docs/manual-verification.md).
5. Atualize a documentação se mudar rotas, variáveis ou comportamento público.

A integração contínua roda `npm ci`, `npx tsc --noEmit`, `npm test` e
`npm run build` em cada push e pull request
([`.github/workflows/ci.yml`](./.github/workflows/ci.yml)).

## Uso Academico e Submissao ao JOSS

O DirrochaCMS está em preparação para submissão ao Journal of Open Source Software (JOSS), com o
enquadramento de arquitetura enxuta para backends serverless em aplicações leves de pesquisa e
ensino.

| Arquivo | Finalidade |
| --- | --- |
| [`paper/paper.md`](./paper/paper.md) | Artigo do JOSS (inglês, versão submetida). |
| [`paper/paper.ptbr.md`](./paper/paper.ptbr.md) | Versão em português do artigo. |
| [`paper/paper.bib`](./paper/paper.bib) | Referências. |
| [`CITATION.cff`](./CITATION.cff) | Metadados de citação. |
| [`docs/joss-readiness.md`](./docs/joss-readiness.md) | Plano de submissão e registro detalhado dos casos de uso. |
| [`docs/joss-readiness.ptbr.md`](./docs/joss-readiness.ptbr.md) | Versão em português do plano de submissão. |
| [`docs/manual-verification.md`](./docs/manual-verification.md) | Roteiro manual de verificação para revisores. |
| [`docs/manual-verification.ptbr.md`](./docs/manual-verification.ptbr.md) | Versão em português do roteiro manual de verificação. |
| [`examples/`](./examples) | Casos de uso documentados (abaixo). |

Três casos de uso documentados:

- [`examples/research-group-backend`](./examples/research-group-backend) — um grupo de pesquisa da
  UFPA usou o DirrochaCMS para publicar informações de projeto e de planejamento semestral numa
  plataforma web. Relatado apenas em alto nível, por ser um deploy de terceiro.
- [`examples/osteoplay-vet`](./examples/osteoplay-vet) — OsteoPlay Vet, TCC de Medicina
  Veterinária da UNAMA Parque Shopping, em Belém/PA. Um jogo educativo em React/Vite consumiu um
  backend DirrochaCMS com 6 endpoints e 66 registros.
- [`examples/charmosinha-makeapi`](./examples/charmosinha-makeapi) — demonstração didática para
  estudantes de Ciência da Computação na disciplina de Engenharia de Software, sob o codinome
  MakeAPI. Um storefront em Next.js consumiu um backend com 4 endpoints e 19 registros. Todo o
  conteúdo é fictício e foi usado apenas para fins educacionais.

Créditos completos dos participantes, esquemas de endpoint observados e contagens de registros
estão em [`docs/joss-readiness.md`](./docs/joss-readiness.md).

Se você usar o DirrochaCMS em trabalho acadêmico, cite-o usando os metadados de
[`CITATION.cff`](./CITATION.cff).

## Changelog

As notas de release são mantidas em [`CHANGELOG.md`](./CHANGELOG.md).
...
Contribuições são bem-vindas. Leia [CONTRIBUTING.pt-BR.md](./CONTRIBUTING.pt-BR.md) antes de
abrir uma issue ou pull request. Issues e pull requests podem ser escritos em português ou
inglês.
...
**Onde pedir ajuda.** Para dúvidas de instalação ou uso do DirrochaCMS, abra uma
[issue no GitHub](https://github.com/marco0antonio0/DirrochaCMS/issues) usando o template de bug
report ou de feature request — esse é o canal de suporte do projeto, e as respostas ficam
pesquisáveis para outras pessoas.

## Codigo de Conduta

Este projeto segue uma política de convivência documentada em
[CODE_OF_CONDUCT.pt-BR.md](./CODE_OF_CONDUCT.pt-BR.md). Esperamos respeito, clareza e
colaboração objetiva em issues, pull requests e discussões.

## Seguranca

Para vulnerabilidades, não abra uma issue pública com detalhes exploráveis. Siga as orientações
em [SECURITY.pt-BR.md](./SECURITY.pt-BR.md).
...
- A sessão usa cookie `HttpOnly` + `SameSite=Lax` + verificação de mesma origem nas escritas.
...
- O login tem limite de taxa por conta e por IP; a API pública tem limite por IP e rota.
- A senha de endpoint privado é guardada como HMAC-SHA256 e nunca é devolvida pela API.
- Os registros são validados contra os campos declarados do endpoint, com limites de tamanho e
  recusa de chaves que poluem protótipo.
...
- Alterações em contas e endpoints são registradas num log de auditoria.
