...
export function InputMultilineComponent({data}:{data:FieldProps}){
  ...
  return(<>
  <h1 className="mb-1 mt-3 text-sm font-medium text-slate-600">{title}</h1>
  <textarea className="h-36 w-full resize-y rounded-lg border-2 border-gray-200 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 smi:px-5" value={value??""} placeholder="digite" onChange={(event) => {
...
export function InputSingleComponent({data}:{data:FieldProps}){
  ...
  return (<>
  <h1 className="mb-1 mt-3 text-sm font-medium text-slate-600">{title}</h1>
  <input type="text" className="h-12 w-full rounded-lg border-2 border-gray-200 px-4 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 smi:h-14 smi:px-5" value={value||""}  placeholder="digite"   onChange={(event) => {
                const newValue = event.target.value;
                const updatedData = [...itemSelected];
...
export function InputSingleNumberComponent({ data }: { data: FieldProps }) {
  ...
  return (
    <>
      ...
      <input
        type="number"
        className="h-12 w-full rounded-lg border-2 border-gray-200 px-4 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 smi:h-14 smi:px-5"
        value={value || ""}
        placeholder="Digite"
        onChange={(event) => {
...
export function InputImageUpload({file,handleDrop,image,title}:{file:any,handleDrop:any,image:any,title:string}) {
  ...
  <h1 className="mb-2 mt-3 text-sm font-medium text-slate-600">{title}</h1>
    <div
      className="mx-auto flex w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 px-4 py-6 text-center transition hover:bg-gray-100"
      onDrop={handleDrop}
      onDragOver={(e) => e.preventDefault()}
      onClick={() => document.getElementById("fileInput")?.click()}
    >
      {image ? (
        <img src={image} alt="Preview" className="h-36 w-36 rounded-md object-cover shadow-md smi:h-40 smi:w-40" />
      ) : (
        <>
          <svg
          ...
          <p className="mt-2 text-sm text-gray-600 smi:text-base">Arraste e solte uma imagem aqui</p>
